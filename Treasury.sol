...
contract Treasury is ContractGuard {
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of QUARTZ price
    ...
    uint256 public mintingFactorForPayingDebt; // print extra QUARTZ during debt phase
    ...
    modifier checkEpoch() {
        ...
        epoch = epoch.add(1);
        epochSupplyContractionLeft = (getUnitePrice() > kittyPriceCeiling)
            ? 0
            : getUniteCirculatingSupply().mul(maxSupplyContractionPercent).div(
                10000
            );
    }
    ...
    function getBurnableUniteLeft()
        public
        view
        returns (uint256 _burnableUniteLeft)
    {
        uint256 _kittyPrice = getUnitePrice();
        if (_kittyPrice <= kittyPriceOne) {
            uint256 _kittySupply = getUniteCirculatingSupply();
            uint256 _bondMaxSupply = _kittySupply.mul(maxDebtRatioPercent).div(
                10000
            );
            uint256 _bondSupply = IERC20(bbond).totalSupply();
            if (_bondMaxSupply > _bondSupply) {
                uint256 _maxMintableBond = _bondMaxSupply.sub(_bondSupply);
                uint256 _maxBurnableUnite = _maxMintableBond
                    .mul(_kittyPrice)
                    .div(1e18);
                _burnableUniteLeft = Math.min(
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_kittyPrice <= kittyPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = kittyPriceOne.mul(1e18).div(_kittyPrice); // to burn 1 QUARTZ
    ...
    function getBondPremiumRate() public view returns (uint256 _rate) {
        uint256 _kittyPrice = getUnitePrice();
        if (_kittyPrice > kittyPriceCeiling) {
            uint256 _kittyPricePremiumThreshold = kittyPriceOne
                .mul(premiumThreshold)
                .div(100);
            if (_kittyPrice >= _kittyPricePremiumThreshold) {
                //Price > 1.10
                uint256 _premiumAmount = _kittyPrice
                    .sub(kittyPriceOne)
                    .mul(premiumPercent)
                    .div(10000);
    ...
    function initialize(
        ...
        kittyPriceOne = 10**18; // This is to allow a PEG of 1 QUARTZ per AVAX
        ...
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn QUARTZ and mint qBOND)
        maxDebtRatioPercent = 4500; // Upto 35% supply of qBOND to purchase
    ...
    function setUnitePriceCeiling(uint256 _kittyPriceCeiling)
        external
        onlyOperator
    {
        require(
            _kittyPriceCeiling >= kittyPriceOne &&
                _kittyPriceCeiling <= kittyPriceOne.mul(120).div(100),
            "out of range"
        ); // [$1.0, $1.2]
    ...
    {
        require(
            _premiumThreshold >= kittyPriceCeiling,
            "_premiumThreshold exceeds kittyPriceCeiling"
        );
        require(
    ...
    function buyBonds(uint256 _kittyAmount, uint256 targetPrice)
        external
        onlyOneBlock
        checkCondition
        checkOperator
    {
        ...
        require(kittyPrice == targetPrice, "Treasury: QUARTZ price moved");
        require(
        ...
        require(
            _kittyAmount <= epochSupplyContractionLeft,
        ...
        require(
            newBondSupply <= kittySupply.mul(maxDebtRatioPercent).div(10000),
            "over max debt ratio"
        );
        ...
        checkOperator
    {
        ...
        require(
            IERC20(kitty).balanceOf(address(this)) >= _kittyAmount,
            "Treasury: treasury has no more budget"
        );

        seigniorageSaved = seigniorageSaved.sub(
            Math.min(seigniorageSaved, _kittyAmount)
        );
    ...
    function _sendToBoardroom(uint256 _amount) internal {
        ...
        if (team1FundSharedPercent > 0) {
            _team1FundSharedAmount = _amount.mul(team1FundSharedPercent).div(
                10000
            );
    ...
    function _calculateMaxSupplyExpansionPercent(uint256 _kittySupply)
        internal
        returns (uint256)
    {
        ...
        checkOperator
    {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            _sendToBoardroom(
                kittySupply.mul(bootstrapSupplyExpansionPercent).div(10000)
            );
        } else {
            if (previousEpochUnitePrice > kittyPriceCeiling) {
                // Expansion ($QUARTZ Price > 1 $ETH): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(bbond).totalSupply();
                uint256 _percentage = previousEpochUnitePrice.sub(
                    kittyPriceOne
                );
                uint256 _savedForBond;
                uint256 _savedForBoardroom;
                uint256 _mse = _calculateMaxSupplyExpansionPercent(kittySupply)
                    .mul(1e14);
                if (_percentage > _mse) {
                    _percentage = _mse;
                }
                if (
                    ...
                    uint256 _seigniorage = kittySupply.mul(_percentage).div(
                        1e18
                    );
